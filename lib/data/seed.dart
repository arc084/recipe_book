...
abstract final class Seed {
  static ({LibraryDatabase library, PantryDatabase pantry}) build() {
    ...
    // Fridge
    final chicken = p('chicken breast',
    ...
    final basil = p('basil', group: PantryGroup.fridge, aliases: ['fresh basil']);
    final butter = p('butter',
        group: PantryGroup.fridge,
        serving: 14,
        alt: 1,
        altUnit: 'tbsp',
    ...
    // Pantry
    final chocChips = p('chocolate chips',
        ...
        carbs: 46,
        sugar: 2);
    final tomatoes = p('canned tomatoes',
    ...
    final onions = p('onions', aliases: ['onion', 'onion, diced', 'brown onion']);
    final chickpeas = p('chickpeas',
    ...
    final salt = p('salt', staple: true, cal: 0, protein: 0, fat: 0, carbs: 0, sugar: 0);
    final pepper = p('pepper',
        staple: true, aliases: ['black pepper'],
        cal: 251, protein: 10.4, fat: 3.3, carbs: 64, sugar: 0.6);
    final sugarItem = p('sugar',
        staple: true, cal: 387, protein: 0, fat: 0, carbs: 100, sugar: 100);
    final flour = p('flour',
        staple: true, aliases: ['plain flour', 'all-purpose flour'],
        cal: 364, protein: 10, fat: 1, carbs: 76, sugar: 0.3);

    // Things other seeded recipes draw on.
    final oats = p('rolled oats',
        aliases: ['oats', 'porridge oats'],
        cal: 379, protein: 13.2, fat: 6.5, carbs: 67.7, sugar: 0.99);
    final yogurt = p('greek yogurt',
        group: PantryGroup.fridge,
        aliases: ['yoghurt', 'greek yoghurt'],
        cal: 59, protein: 10, fat: 0.4, carbs: 3.6, sugar: 3.2);
    final milk = p('milk',
        group: PantryGroup.fridge,
        serving: 240, alt: 1, altUnit: 'cup',
        cal: 42, protein: 3.4, fat: 1, carbs: 5, sugar: 5);
    final tuna = p('sushi-grade tuna',
        group: PantryGroup.freezer,
        aliases: ['tuna', 'ahi tuna'],
        cal: 144, protein: 23.3, fat: 4.9, carbs: 0, sugar: 0);
    final rice = p('sushi rice',
        aliases: ['rice', 'short-grain rice'],
        cal: 358, protein: 6.5, fat: 0.6, carbs: 79, sugar: 0.1);
    final soy = p('soy sauce',
        aliases: ['shoyu'],
        serving: 18, alt: 1, altUnit: 'tbsp',
        cal: 53, protein: 8.1, fat: 0.6, carbs: 4.9, sugar: 0.4);
    final curryRoux = p('curry roux',
        aliases: ['japanese curry roux', 'curry blocks'],
        pack: 220, packUnit: 'g',
        cal: 474, protein: 5.9, fat: 30, carbs: 45, sugar: 12,
        estimated: true);
    final potato = p('potatoes',
        aliases: ['potato'],
        serving: 170, alt: 1, altUnit: 'potato',
        cal: 77, protein: 2, fat: 0.1, carbs: 17, sugar: 0.8);
    final carrot = p('carrots',
        group: PantryGroup.fridge,
        aliases: ['carrot'],
        serving: 61, alt: 1, altUnit: 'carrot',
        cal: 41, protein: 0.9, fat: 0.2, carbs: 10, sugar: 4.7);
    final coconutMilk = p('coconut milk',
        pack: 400, packUnit: 'ml',
        cal: 197, protein: 2, fat: 21, carbs: 3, sugar: 3);
    final spinach = p('spinach',
        group: PantryGroup.fridge,
        cal: 23, protein: 2.9, fat: 0.4, carbs: 3.6, sugar: 0.4);
    final chiaSeeds = p('chia seeds',
        cal: 486, protein: 16.5, fat: 30.7, carbs: 42.1, sugar: 0);
    final maple = p('maple syrup',
        serving: 20, alt: 1, altUnit: 'tbsp',
        cal: 260, protein: 0, fat: 0.1, carbs: 67, sugar: 60);
    final avocado = p('avocado',
        group: PantryGroup.fridge,
        serving: 150, alt: 1, altUnit: 'avocado',
        cal: 160, protein: 2, fat: 15, carbs: 9, sugar: 0.7);
    final cucumber = p('cucumber',
        group: PantryGroup.fridge,
        cal: 15, protein: 0.7, fat: 0.1, carbs: 3.6, sugar: 1.7);
    final sesameOil = p('sesame oil',
        serving: 13.6, alt: 1, altUnit: 'tbsp',
        cal: 884, protein: 0, fat: 100, carbs: 0, sugar: 0);
    final paprika = p('smoked paprika',
        staple: false,
        serving: 2.3, alt: 1, altUnit: 'tsp',
        cal: 282, protein: 14.1, fat: 13, carbs: 54, sugar: 10.3);
    final cumin = p('cumin',
        serving: 2.1, alt: 1, altUnit: 'tsp',
        cal: 375, protein: 17.8, fat: 22.3, carbs: 44.2, sugar: 2.3);

    final pantryItems = <PantryItem>[
      chicken, eggs, mozzarella, parmesan, basil, butter,
      chocChips, tomatoes, oliveOil, cocoa, onions, chickpeas,
      salmon, peas, pastry,
      salt, pepper, sugarItem, flour,
      oats, yogurt, milk, tuna, rice, soy, curryRoux, potato, carrot,
      coconutMilk, spinach, chiaSeeds, maple, avocado, cucumber, sesameOil,
      paprika, cumin,
    ];
    ...
    Recipe recipe({
      ...
      int cooked = 0,
      String notes = '',
      required List<(String, List<(double?, String, String, PantryItem?)>,
              List<String>)>
          parts,
    }) {
      final r = Recipe(
    ...
    final chickenParm = recipe(
      ...
      minutes: 40,
      cooked: 4,
      notes: 'Sauce is better made the day before. Half quantities fit one '
